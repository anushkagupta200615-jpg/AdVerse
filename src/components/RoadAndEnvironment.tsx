...
function useAdTexture(textureUrl: string | null, lastPrompt?: string, lastBidAmount?: string) {
  const fallbackLabel = 'Local User'
  const fallbackAmount = lastBidAmount ? `$${Number(lastBidAmount).toFixed(3)}` : 'LIVE BID'
  const [texture, setTexture] = useState<THREE.Texture>(() => makeBillboardTexture(fallbackLabel, 'agent-bid dynamic ad', fallbackAmount))

  useEffect(() => {
    let cancelled = false
    const fallback = makeBillboardTexture(fallbackLabel, lastPrompt ?? 'agent-bid dynamic ad', fallbackAmount)
  ...
  }, [textureUrl, fallbackLabel, fallbackAmount, lastPrompt])
...
export default function RoadAndEnvironment({ textureUrl, lastPrompt, lastBidAmount, scene }: RoadAndEnvironmentProps) {
  const theme = scenes[scene]
  const adTexture = useAdTexture(textureUrl, lastPrompt, lastBidAmount)
